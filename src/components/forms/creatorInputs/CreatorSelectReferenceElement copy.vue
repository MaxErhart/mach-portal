  ...
  <div class="creator-input-element" :class="{edit: edit}">
    ...
    <div class="edit-element" :class="{active: edit}">
      ...
      <section class="source-select">
        <SelectElement :data="sourceSelectData" :name="`${name}_formId_data`" :dynamic="false" @selectedEntry="selectSourceForm($event)" :presetValue="sourceForm==null ? null : sourceForm.name"/>
      </section>
      <section class="source-display" v-if="sourceForm!==null">
        <SelectElement :data="sourceDisplayData" :name="`${name}_displayId_data`" :dynamic="false" :nameCast="(v)=>v.data.label" @selectedEntry="selectSourceDisplay($event)" :presetValue="sourceDisplay==null ? null : sourceDisplay.data.label"/>
      </section>
      <section>
...
export default {
  ...
  methods: {
    ...
    async getForms() {
      this.awaitData = true
      const {forms, error} = await this.$store.dispatch('forms')
      if(error) {
        return
      }
      this.forms = forms.filter(form=>form.submit_permission>0)
