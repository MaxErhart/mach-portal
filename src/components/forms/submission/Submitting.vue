<template>
<div>
  <div v-if="form && groups && form.organization_proxy" class="select-organization">
    <div class="select-organization-body">
      <h2>Please select group/organization</h2>
      <p>This form requires users to work on behalf of one group/organization.</p>
      <p>This is necessary such that in the event of useres changing or leaving an organization their work will still be associated with that organization.</p>
      <SelectElement :search="false" label="Select Organization" :required="true" :data="organizations" @selectedEntry="selectOrganization($event)" :presetValue="selected_organization?.id"/>
    </div>
 </div>
  <div class="submitting" v-if="form && groups && (!form.organization_proxy || selected_organization)">
    <div class="submitting-body">
      ...
      <form class="form" @submit.prevent="update ? postUpdate($event) : submit($event)" ref="form">
        ...
        <section class="owner-section" :style="{'display': (is_public || form.organization_proxy) && !user.groups.find(g=>g.id==25)?'none':'block'}" v-if="!is_public" >
          <div class="select-wrapper"   :style="{'display': !show_owner_select?'none':'block'}">
...
export default {
  ...
  computed: {
    organizations() {
      const organizations = []
      this.form.organizations_regex.forEach(regex_string=>{
        const regex = new RegExp(regex_string)
        this.groups.forEach(group=>{
          if(regex.test(group.name)) {
            organizations.push({id:group.id,name:group.name})
          }
        })
      })
    ...
    current_owner_option_id() {
      if(this.form?.organization_proxy && this.selected_organization) {
        return `group.${this.selected_organization.id}`
      }
  ...
  methods: {
    setOrganization() {
      if(this.organizations.length===1) {
        this.selected_organization = this.groups.find(g=>g.id==this.organizations[0].id)
      }
    },
    selectOrganization(event) {
      this.selected_organization = this.groups.find(group=>group.id==event?.id)
    },
    ...
    async getGroups() {
      ...
      console.log(error?.response)
      this.groups = groups
      if(this.form.organization_proxy) {
