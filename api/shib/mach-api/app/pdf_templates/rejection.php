...
function create($bewerber=NULL, $lang='de') {
    ...
    // $legal = 'Amtliche Bekanntmachung des KIT Nr. 68 vom 24. November 2017';
    ...
    $pdf->HTMLCell(487.5, 17.3, utf8_decode("Ihr Antrag vom {$entrance_exam_registration_changed} auf Zulassung zur Aufnahmeprüfung für den {$degree} wird gemäß Anlage 1, Nummer 2.3 der Satzung für den Zugang zu dem {$degree} am Karlsruher Institut für Technologie (KIT) ({$legal}) <b>abgelehnt</b>."), 0, 'J', 0);



    // BODY TEXT CONTINUE
    $pdf->SetFont('Helvetica','',10);
    $pdf->SetXY(65.08, $pdf->GetY()+$line_height); // set the position of the box
    ...
    // BODY FIN
    $pdf->SetXY(65.08, $pdf->GetY()+2*$line_height); // set the position of the box
    $pdf->HTMLCell(487.5, 17.3, utf8_decode("Mit freundlichen Grüßen<br><br>Zugangskommission für den $degree<br>KIT-Fakultät für Maschinenbau*"), 0, 'J', 0);
    ...
    if (preg_match('/^[\/\w\-. ]+$/', $name)) {
        $filename = "rejection_{$bewerbungs_nummer}_{$name}_{$today}_{$uniqid}.pdf";
    } else {
        $filename = "rejection_{$bewerbungs_nummer}_{$today}_{$uniqid}.pdf";
    } 
