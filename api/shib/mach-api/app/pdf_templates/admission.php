<?php
namespace PdfTemplates\Admission;
...
function create($bewerber=NULL, $lang='de') {
    ...
    $pdf->MultiCell(487.5, 17.3, utf8_decode("Ihrem Antrag vom {$entrance_exam_registration_changed} auf Zulassung zur Aufnahmeprüfung für den {$degree} gemäß Satzung für den Zugang zu dem {$degree} am Karlsruher Institut für Technologie (KIT) ({$legal}) wird stattgegeben."), 0, 'J', 0);
    ...
    // BODY TEXT CONTINUE
    $pdf->SetFont('Helvetica','',10);
    $pdf->SetXY(65.08, 488); // set the position of the box
    $pdf->MultiCell(487.5, 17.3, utf8_decode('Weitere Informationen zur Online-Prüfung sowie Ihre Registrierungsdaten zur Prüfungsplattform werden Ihnen zeitnah zugehen.'), 0, "J", 0); // add the text, align to Center of cell
    
    // BODY FIN
    $pdf->SetXY(65.08, 543.2); // set the position of the box
    $pdf->Cell(0, 0, utf8_decode('Mit freundlichen Grüßen'), 0, 0, "L", 1); // add the text, align to Center of cell

    $pdf->SetXY(65.08, 560.2); // set the position of the box
    $pdf->Cell(0, 0, utf8_decode("Zugangskommission für den $degree"), 0, 0, "L", 1); // add the text, align to Center of cell

    $pdf->SetXY(65.08, 577.2); // set the position of the box
