...
class ArchiveController extends Controller
{
    ...
    private function getSubfoldersWithPermission($sub_folders,$root,$user) {
        ...
        $scan_dirs = scandir($folder);
        // return [$fragment,$folder];
        $dirs_result = [];
        $files_result = [];
        // return $root_path;

        // return $user->archive_permissions_condensed();
        foreach($user->archive_permissions_condensed() as $level=>$dirs) {
            foreach($dirs as $dir=>$permission) {
                foreach($scan_dirs as $scan_dir) {
                    if(!$this->isValidDir($scan_dir)) {
                        continue;
                    }
                    if(!str_starts_with(strtolower($folder."/".$scan_dir),strtolower($root_path.$dir))) {
                        continue;
                    }
                    if($fragment) {
                        $curr_fragments = $fragment."/".$scan_dir; 
                    } else {
                        $curr_fragments = $scan_dir; 
                    }
                    if(array_key_exists($folder."/".$scan_dir,$dirs_result) && $permission>$dirs_result[$folder."/".$scan_dir]["permission"]) {
                        $dirs_result[$folder."/".$scan_dir]["permission"] = $permission;
                    }
                    if(is_dir($folder."/".$scan_dir)){
                        $dirs_result[$folder."/".$scan_dir] = [
        ...
        return ["folders"=>array_values($dirs_result),"files"=>$files_result];

        // foreach($user->archive_permissions_condensed() as $level=>$permission) {
