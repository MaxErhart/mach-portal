...
function create($submissions,$elements,$ref_subs,$output_file,$prof,$institut,$semester,$settings) {
  ...
  $id_order = $settings->compound_form_data["id_order"];
  $insg_total = 0;
  $prof_total = 0;
  $mitarb_total = 0;  
  ...
  while($make_table) {
    $make_table_return_data = makeTable($pdf,$table_submissions,$page_width,$table_start_y,$text_padding,$page_padding_x,$col_width,$page_height,$footer_height,$elements,$insg_total,$prof_total,$mitarb_total,$pages,$ref_subs,$id_order);
...
function makeTable($pdf,$submissions,$page_width,$table_start_y,$text_padding,$page_padding_x,$col_width,$page_height,$footer_height,$elements,$insg_total,$prof_total,$mitarb_total,$pages,$ref_subs,$id_order) {
  $new_page=false;
  $index=0;
  $table_header_end = makeTableHeader($pdf,$table_start_y,$text_padding,$page_padding_x,$col_width);

  $table_fill_return_data = fillTable($pdf,$submissions,$table_header_end,$text_padding,$page_padding_x,$col_width,$page_height,$footer_height,$elements,$ref_subs,$id_order);
...
function fillTable($pdf, $submissions,$table_header_end,$text_padding,$page_padding_x,$col_width,$page_height,$footer_height,$elements,$ref_subs,$id_order=null,$index=0) {
  // abort(response()->json($id_order,401));
