<template >
  <template v-if="data">
    <div id="element-body" ref="body" :class="{active: selectActive}">
      ...
      <InputElement @focus="openSelect()" @blur="blur()" :readonly="!typeToSearch" :label="label" type="text" :required="required" :placeholder="placeholder" :tooltip="tooltip" :autocomplete="autocomplete ? data.autocomplete:'off'" @valueChange="inputChanged($event)" :height="height" class="input-select" :presetValue="selected ? nameCast(selected) : inputElementValue" @click="openSelect()" ref="input"/>
